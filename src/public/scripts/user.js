import { getUserId, selectById, selectByValue } from "../main.js";
...
async function populateRequests(requests) {
    ...
    for (const request of requests) {
        ...
        if (!project) throw new Error("request references void project");
...
function populateTags(memberships) {
    const div = document.getElementById("user_tags");
    ...
    // Add one tag for the role that we have
    for (const role of role_list)
        div.innerHTML += `<span class="badge badge-secondary">${role}</span>`;
}
